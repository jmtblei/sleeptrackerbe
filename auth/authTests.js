    ...
    describe('auth tests', () => {
        ...
        describe('POST /register endpoint tests', () => {
            ...
            it('return 401 status if username or password are not provided or blank', async () => {
                let response = await request(server).post('/api/auth/register').send({ usernae: 'username', password: 'password' });
                expect(response.status).toEqual(401);
                response = await request(server).post('/api/auth/register').send({ username: '', password: '' });
                expect(response.status).toEqual(401);
            })

            it('return a message error if username or password are not provided or blank', async () => {
                let response = await request(server).post('/api/auth/register').send({ username: '', password: '' });
                expect(response.body).toEqual({ message: 'You need to send an username and a password' });
                response = await request(server).post('/api/auth/register').send({ usernme: 'asd', password: 'asd' });
                expect(response.body).toEqual({ message: 'You need to send an username and a password' });
            })

            it('return a message error if email has wrong format', async () => {
                const response = await request(server).post('/api/auth/register').send({ username: 'username', password: 'password', email: "addasd" });
        ...
        describe('POST /login enpoint tests', () => {
            it('return 200 status if a user is logged in correctly', async () => {
                await request(server).post('/api/auth/register').send({ username: 'username', password: 'password' });
                const response = await request(server).post('/api/auth/login').send({ username: 'username', password: 'password' });
            ...
            it('return 401 status if username or password are not provided or wrong', async () => {
                let response = await request(server).post('/api/auth/login').send({ username: '', password: '' });
                expect(response.status).toEqual(401);
                response = await request(server).post('/api/auth/login').send({ username: 'asd', password: 'asd' });
                expect(response.status).toEqual(401);
            })
